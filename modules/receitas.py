...
def listar_contas(usuario_id):
    response = supabase.table("contas").select("*").eq("usuario_id", usuario_id).order("nome").execute()
...
def criar_receita(usuario_id, mes, descricao, categoria, conta_id, valor, recebida=True):
    ...
    if recebida and conta_id:
        conta = supabase.table("contas").select("saldo").eq("id", conta_id).single().execute()
...
def listar_receitas(usuario_id, mes):
    response = supabase.table("receitas").select("*").eq("usuario_id", usuario_id).eq("mes", mes).order("id", desc=True).execute()
    return response.data
...
def buscar_nome_conta(conta_id):
    ...
    conta = supabase.table("contas").select("nome").eq("id", conta_id).single().execute()
...
def deletar_receita(usuario_id, receita_id):
    receita = supabase.table("receitas").select("*").eq("id", receita_id).eq("usuario_id", usuario_id).single().execute()
    receita_data = receita.data

    if receita_data and receita_data["recebida"] and receita_data["conta_id"]:
        conta = supabase.table("contas").select("saldo").eq("id", receita_data["conta_id"]).single().execute()
